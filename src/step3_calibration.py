...
# Output
OUTPUT_FILE = os.path.join(DATA_DIR, "step3_calibration.csv")
CALIBRATION_JSON = os.path.join(DATA_DIR, "calibration_factors.json")
OUTPUT_XLSX = os.path.join(DATA_DIR, "step3_calibration_report.xlsx")
...
# --- Simulation Loop (RETRY + ROBUST PARSE) ---
    ...
    if results:
        ...
        with open(CALIBRATION_JSON, 'w') as f:
            json.dump(calibration_factors, f, indent=4)
        print(f"-> Đã lưu file hệ số Delta: {CALIBRATION_JSON}")
